<template>
  ...
  >
    ...
    <!-- Inhalt -->
    <div :class="compact ? 'flex-1 min-w-0' : 'p-4 pb-0 flex-1'">
      <a
        :href="bookmark.url"
        target="_blank"
        rel="noopener"
    ...
    <div
      :class="[
        ...
        compact
          ? 'shrink-0 pl-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200'
          : 'justify-between border-t border-gray-100 dark:border-gray-800 mx-4 pb-3 pt-2'
      ]"
    >
      <!-- Drag Handle (links) -->
      <div
        ...
        class="p-1.5 rounded-lg text-gray-300 hover:text-gray-500 dark:text-gray-600 dark:hover:text-gray-400 cursor-grab active:cursor-grabbing transition"
        :title="t('bookmark.drag')"
      >
...
function onDragStart(event: DragEvent) {
  if (event.dataTransfer) {
    ...
    // Ganze Karte als Drag-Preview anzeigen
    if (cardRef.value) {
      const rect = cardRef.value.getBoundingClientRect();
      const offsetX = event.clientX - rect.left;
      const offsetY = event.clientY - rect.top;
      event.dataTransfer.setDragImage(cardRef.value, offsetX, offsetY);
    }
  }
}
</script>
